...
const  bitField configBits[] =
{
	...
	{ &bitParams, sOnOff, 0, 1, 1 },
	   // BACKLIGHT
	{ &bitParams, sOnOff, 1, 1, 1 },
	   // REL MODE
	{ &bitParams, sLCMode, 2, 1, 1 },
	   // LC MODE
	{ &bitParams, sRLim, 3, 2, 2 },
	   // LIMIT RANGE
	{ &bitParams, sOnOff, 5, 1, 1 }, 
	  // UART MODE
	{ &bitParams, sScrMode, 6, 2, 3 }, 
	  // SCR MODE
	{ &bitParams, NULL, 8, 5, 31 }, 
	  // SCR CONTRAST
	{ &bitParams, NULL, 13, 5, 20 },
	  // AVERAGES
	{ &bitParams, sParamMode, 18, 3, 6 },
	  // 1st param
	{ &bitParams, sParamMode, 21, 3, 6 }, 
	 // 2nd param
	{ &bitParams, sLowBat, 24, 2, 3 },
	  // low bat
	{ &bitParams, sParamMode, 26, 3, 6 },
	  // 3rd param
};
...
const  menuItem menu[] =  {
	{ leafProcess, NULL, MENU_MAIN, 0, 0, "MEAS DISPLAY", -1 },
	  //0
	{ menuProcess, NULL, MENU_DISP, 2, 13, "MAIN MENU", 200 },
	  //1
	{ menuProcess, editContrast, MENU_MAIN, 0, 0, "CONTRAST", -1 },
	  //2
	{ menuProcess, editScr, MENU_MAIN, 0, 0, "DISP MODE", -1 },
	  //3
	{ menuProcess, editBF, MENU_MAIN, BIT_BL_MODE, 0, "BACKLIGHT", -1 },
	  //4
	{ menuProcess, editAverages, MENU_MAIN, 0, 0, "AVERAGES", -1 },
	  //5
	{ menuProcess, editR, MENU_MAIN, 0, 0, "SHUNT R", -1 },
	  //6
	{ menuProcess, editBF, MENU_MAIN, BIT_LC_MODE, 0, "LC EQUIV", -1 },
	  //7
	{ menuProcess, editBF, MENU_MAIN, BIT_LIM_RANGE, 0, "RANGE LIM", -1 },
	  //8
	{ menuProcess, editBF, MENU_MAIN, BIT_1STPARAM, 0, "PARAM 1", -1 }, 
	 //9
	{ menuProcess, editBF, MENU_MAIN, BIT_2NDPARAM, 0, "PARAM 2", -1 },
	  //10
	{ menuProcess, editBF, MENU_MAIN, BIT_3RDPARAM, 0, "PARAM 3", -1 },
	  //11
	{ menuProcess, editBF, MENU_MAIN, BIT_UART_MODE, 0, "UART OUT", -1 },
	  //12
	{ menuProcess, editBF, MENU_MAIN, BIT_LOWBAT, 0, "LOW BAT", -1 }, 
	 //13

	{ menuProcess, NULL, MENU_DISP, 15, 17, "CALIBRATION", 200 }, 
	//14
	{ leafBalance, editBalance, MENU_DISP, 15, 0, "CH BALANCE", -1 }, 
	 //15
	{ leafCal, editCal, MENU_DISP, 0, 0, "OPEN-SHORT", -1 }, 
	 //16
	{ leafCalReset, NULL, MENU_CAL, 0, 0, "RESET", -1 },
	  	   //17

	{ leafInfo, NULL, MENU_DISP, 0, 0, "Info", 20 },
	   //18
	{ leafScr, NULL, MENU_DISP, 0, 0, "scrMode", -1 },
	   //19
	{ leafBatLow, NULL, MENU_DISP, 0, 0, "lowBat", 20 },
	   //20
	{ diagScr, NULL, MENU_DISP, 0, 0, "diagScr", -1 },
	   //21
};
...
void puthex(int inp, short size)
{
	...
	{
		char dig = (inp & 0xf);
		if (dig < 10){buf[j] = 0x30 + dig; }
		else{buf[j] = 0x41 + dig - 10; }
		inp >>= 4;
	}
...
//----------------------------------------------------------------------------
float filter(int sidx, float newVal)
{
	static float state[4];
	static int cnt[4];

	float t = (state[sidx] - newVal);
	state[sidx] = state[sidx]*(1 - ALPHA) + newVal*ALPHA;
	if (t < 0)cnt[sidx]++;
	else  cnt[sidx]--;
	...
	if ((btns) & BTN_PUSH) // menu select
		{
			if (editable == 0)
			{
				...
				{
					if (menu[selected].d(selected, tmpStr, 0)){return selected;}
					else{editable = 1; }
				}
				else
					return selected;
...
int leafProcess(int btns)
{
	...
	if (round >= averages)
	{
		...
		{
			printFloat(15, 2, Z.Re, "\\");

			if (Z.Im > 0)
			{
				printFloat(15, 5, ls, "H");
			}
			else
			{
				printFloat(15, 5, cs, "F");
			}
		}
		else
		{
	...
	printBat(100 - 2, 0, (((int)vbat - BAT_MIN) * 100) / (BAT_MAX - BAT_MIN));

	lcd_gotoxy(100, 3);
	lcd_putstr(sFreqs[findex], 0);

	lcd_gotoxy(100, 4);
	if ((cstatus & (1 << (findex + 8))))	lcd_putstr("BAL", 0);
	else	lcd_putstr("---", 0);

	lcd_gotoxy(100, 5);
	if ((cstatus & (1 << findex))) lcd_putstr("CAL", 0);
	else	lcd_putstr("---", 0);
	...
	lcd_gotoxy(100, 7);
	if (getBf(BIT_REL_MODE)) lcd_putstr(">-<", 0);
	else	lcd_putstr("   ", 0);
...
{
	lcd_gotoxy(0, 0); lcd_putstr("      6.20b12     ", 1);
	lcd_gotoxy(0, 1); lcd_putstr("     Neekeetos    ", 0);
	lcd_gotoxy(0, 2); lcd_putstr("    @yahoo.com    ", 0);
	lcd_gotoxy(0, 3); lcd_putstr("   big thanks to  ", 0);
	lcd_gotoxy(0, 4); lcd_putstr("  TESLight, Link  ", 0);
	lcd_gotoxy(0, 5); lcd_putstr(" Ozzy & radiocats ", 0);
	lcd_gotoxy(0, 6); lcd_putstr("  @ radiokot.ru   ", 0);
	lcd_gotoxy(0, 7); lcd_putstr("OLED 1.3\" version ", 0);

	if ((menu[menuCurrent].timeout < (hundredMsTick - startTime)))
	{
		return menu[menuCurrent].parent ;    // menu select
	}
...
int diagScr(int btns) {
	...
	lcd_putstr(sFreqs[findex], 0);
	if (sigma)lcd_putstr("   SIGMA", 1);
	 else lcd_putstr("    MEAN", 1);
	lcd_gotoxy(6 * 13, 1);
	if (rel)lcd_putstr("REL", 1);
	 else lcd_putstr("    ", 1);
	...
	if (btns & (BTN_PUSH << REL_BTN)){ 	if (sigma) sigma = 0;
		else sigma = 1; }
	if (btns & (BTN_LPUSH << REL_BTN)){ 	if (rel) rel = 0;
		else rel = 1; }
	if (btns & (BTN_PUSH << SP_BTN)) {
		findex++;
		if (findex >= FCNT) findex = 0;
...
int editBalance(int sel, char* str, int btns) {
	...
	if (cstatus & (1 << (findex + 8))) strcpy(str, "OK");
	 else strcpy(str, "--");
...
int editCal(int sel, char* str, int btns) {
	...
	if (cstatus & (1 << (findex))) strcpy(str, "OK"); 
	else strcpy(str, "--");
